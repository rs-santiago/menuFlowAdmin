...
export default function ProductForm() {
  ...
  // 1. Carrega Categorias e dados do produto (se for edição)
  useEffect(() => {
      ...
      try {
        ...
        if (productId) {
          ...
          setForm({
            ...
            price: String(p.price), // Transforma de volta pra string pro TextInput
  ...
  const handleSave = async () => {
    if (!form.name || !form.price || !form.categoryId) {
      Alert.alert("Campos obrigatórios", "Por favor, preencha nome, preço e categoria.");
      return;
    }
    ...
    try {
      ...
      if (imageUri && !imageUri.startsWith('http')) {
        try {
          ...
          console.log('Erro detalhado no front:', error);
          throw error;
        }
      }
      ...
      Alert.alert("Sucesso", "O cardápio foi atualizado!", [{ text: "Boa!", onPress: () => router.back() }]);
    } catch (e: any) {
      console.error(e);
      Alert.alert("Erro", "Não conseguimos salvar o produto.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['bottom', 'left', 'right']}>
      ...
      <KeyboardAvoidingView 
        ...
        keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 20} // Compensa a altura do Header
      >
        <ScrollView 
        ...
        >
          ...
          {/* Seletor de Imagem e resto do seu formulário... */}
